...
export const createTeam = async ({ name, description, userId }) => {
  ...
  return { id: teamId, name, description: null, userId, role: 'admin' };
};
...
export const findUserByEmail = async (email) => {
  const [rows] = await pool.query('SELECT User_ID as id, Email_ID as email, User_Name as name FROM Users WHERE Email_ID = ?', [email]);
  if (rows.length === 0) return null;
  return rows[0];
};
...
export const addMember = async (teamId, userId, role) => {
  try {
    ...
    const [rows] = await pool.query('SELECT User_Name as name, Email_ID as email FROM Users WHERE User_ID = ?', [userId]);
    return { id: userId, name: rows[0].name, email: rows[0].email, role };
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') return { alreadyMember: true };
    throw err;
  }
};

export const countAdmins = async (teamId) => {
  const [rows] = await pool.query('SELECT COUNT(*) as count FROM Membership WHERE Team_ID = ? AND Is_Admin = 1', [teamId]);
  return rows[0].count;
};

export const updateMemberRole = async (teamId, userId, role) => {
  const isAdmin = role === 'admin' ? 1 : 0;
  await pool.query('UPDATE Membership SET Is_Admin = ? WHERE Team_ID = ? AND User_ID = ?', [isAdmin, teamId, userId]);
};

export const removeMember = async (teamId, userId) => {
  await pool.query('DELETE FROM Membership WHERE Team_ID = ? AND User_ID = ?', [teamId, userId]);
};
